...
namespace AI_Scoreplat
{
    public partial class finallyscore : Form
    {
        ...
        {
            string sql = "SELECT t2.papercode paperno,t2.encodeno,t2.pcid,t2.totalscore score,t2.Itemcount,concat(t1.filepath,'.zip') zippath  FROM AI_task t1, (SELECT papercode,encodeno,pcid,count(1) Itemcount,sum(planscore) totalscore,taskid FROM AI_score WHERE  validflag='2' GROUP BY papercode,encodeno,pcid,taskid) t2 WHERE t1.taskid=t2.taskid";
        ...
        {
            string sql = "SELECT t1.papercode paperno,t1.encodeno ,t1.pcid,replace(t1.itemcode,'ans.mp3','') audiopath,t1.planscore score,concat(t2.filepath,'.zip') zippath FROM AI_score t1,AI_task t2 WHERE t1.taskid=t2.taskid AND t1.validflag='2'";
